...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers.Maestros
{
    ...
    {
        public ActionResult AdministrarTipoNave()
        {
            return View("../Maestros/TipoNave/AdministrarTipoNave");
        }

        public ActionResult RegistroTipoNave()
        {
            return View("../Maestros/TipoNave/RegistroTipoNave");
        }

        public ActionResult BuscarTipoNave()
        {
            return View("../Busqueda/BuscarTipoNave");
        }

        public ActionResult ConsultaTipoNave(RequestBusquedaTipoNaveViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaTipoNave(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaTipoNave,
                            filtros.paginacionDTO.IdGrilla, "CodigoTipoNave", Request.QueryString["export"], Response, "Lista_de_TipoNave_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaTipoNave(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaTipoNave, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoTipoNave");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaTipoNave, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarTipoNave(RequestRegistrarTipoNaveViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarTipoNave(request);
