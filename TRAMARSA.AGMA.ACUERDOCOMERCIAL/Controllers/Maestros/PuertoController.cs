...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers
{
    ...
    {
        public ActionResult AdministrarPuerto()
        {
            return View("../Maestros/Puerto/AdministrarPuerto");
        }

        public ActionResult RegistroPuerto()
        {
            return View("../Maestros/Puerto/RegistroPuerto");
        }

        public ActionResult BuscarPuerto()
        {
            return View("../Busqueda/BuscarPuerto");
        }


        public ActionResult ConsultaPuerto(RequestBusquedaPuertoViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaPuerto(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaPuerto,
                            filtros.paginacionDTO.IdGrilla, "CodigoPuerto", Request.QueryString["export"], Response, "Lista_de_Puerto_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaPuerto(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaPuerto, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoPuerto");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaPuerto, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarPuerto(RequestRegistrarPuertoViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarPuerto(request);
