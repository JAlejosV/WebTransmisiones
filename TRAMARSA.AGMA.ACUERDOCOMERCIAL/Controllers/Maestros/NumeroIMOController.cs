...
{
    public class NumeroIMOController : Controller
    {
        public ActionResult AdministrarNumeroIMO()
        {
            return View("../Maestros/NumeroIMO/AdministrarNumeroIMO");
        }

        public ActionResult RegistroNumeroIMO()
        {
            return View("../Maestros/NumeroIMO/RegistroNumeroIMO");
        }
        ...
        public ActionResult ConsultaNumeroIMO(RequestBusquedaNumeroIMOViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaNumeroIMO(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaNumeroIMO,
                            filtros.paginacionDTO.IdGrilla, "CodigoNumeroIMO", Request.QueryString["export"], Response, "Lista_de_NumeroIMO_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaNumeroIMO(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaNumeroIMO, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoNumeroIMO");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaNumeroIMO, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarNumeroIMO(RequestRegistrarNumeroIMOViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarNumeroIMO(request);
