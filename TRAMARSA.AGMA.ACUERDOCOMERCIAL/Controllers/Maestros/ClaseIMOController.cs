...
{
    public class ClaseIMOController : Controller
    {
        public ActionResult AdministrarClaseIMO()
        {
            return View("../Maestros/ClaseIMO/AdministrarClaseIMO");
        }

        public ActionResult RegistroClaseIMO()
        {
            return View("../Maestros/ClaseIMO/RegistroClaseIMO");
        }
        ...
        public ActionResult ConsultaClaseIMO(RequestBusquedaClaseIMOViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaClaseIMO(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaClaseIMO,
                            filtros.paginacionDTO.IdGrilla, "CodigoClaseIMO", Request.QueryString["export"], Response, "Lista_de_ClaseIMO_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaClaseIMO(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaClaseIMO, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoClaseIMO");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaClaseIMO, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarClaseIMO(RequestRegistrarClaseIMOViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarClaseIMO(request);
