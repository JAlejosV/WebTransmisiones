...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers.Maestros
{
    ...
    {
        public ActionResult AdministrarNave()
        {
            return View("../Maestros/Nave/AdministrarNave");
        }

        public ActionResult RegistroNave()
        {
            return View("../Maestros/Nave/RegistroNave");
        }

        public ActionResult BuscarNave()
        {
            return View("../Busqueda/BuscarNave");
        }

        public ActionResult ConsultaNave(RequestBusquedaNaveViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaNave(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaNave,
                            filtros.paginacionDTO.IdGrilla, "CodigoNave", Request.QueryString["export"], Response, "Lista_de_Nave_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaNave(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaNave, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoNave");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaNave, 0, 0, 0, ""));
                        }
                    }
                }
                else
