...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers.Maestros
{
    ...
    {
        public ActionResult AdministrarTipoContenedor()
        {
            return View("../Maestros/TipoContenedor/AdministrarTipoContenedor");
        }

        public ActionResult RegistroTipoContenedor()
        {
            return View("../Maestros/TipoContenedor/RegistroTipoContenedor");
        }

        public ActionResult BuscarTipoContenedor()
        {
            return View("../Busqueda/BuscarTipoContenedor");
        }

        public ActionResult ConsultaTipoContenedor(RequestBusquedaTipoContenedorViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaTipoContenedor(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaTipoContenedor,
                            filtros.paginacionDTO.IdGrilla, "CodigoTipoContenedor", Request.QueryString["export"], Response, "Lista_de_TipoContenedor_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaTipoContenedor(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaTipoContenedor, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoTipoContenedor");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaTipoContenedor, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarTipoContenedor(RequestRegistrarTipoContenedorViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarTipoContenedor(request);
