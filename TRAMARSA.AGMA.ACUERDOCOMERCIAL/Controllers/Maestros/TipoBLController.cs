...
{
    public class TipoBLController : Controller
    {
        public ActionResult AdministrarTipoBL()
        {
            return View("../Maestros/TipoBL/AdministrarTipoBL");
        }

        public ActionResult RegistroTipoBL()
        {
            return View("../Maestros/TipoBL/RegistroTipoBL");
        }

        public ActionResult BuscarTipoBL()
        {
            return View("../Busqueda/BuscarTipoBL");
        }

        public ActionResult ConsultaTipoBL(RequestBusquedaTipoBLViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaTipoBL(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaTipoBL,
                            filtros.paginacionDTO.IdGrilla, "CodigoTipoBL", Request.QueryString["export"], Response, "Lista_de_TipoBL_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaTipoBL(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaTipoBL, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoTipoBL");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaTipoBL, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarTipoBL(RequestRegistrarTipoBLViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarTipoBL(request);
