...
{
    public class UnidadMercanciaController : Controller
    {
        public ActionResult AdministrarUnidadMercancia()
        {
            return View("../Maestros/UnidadMercancia/AdministrarUnidadMercancia");
        }

        public ActionResult RegistroUnidadMercancia()
        {
            return View("../Maestros/UnidadMercancia/RegistroUnidadMercancia");
        }
        ...
        public ActionResult ConsultaUnidadMercancia(RequestBusquedaUnidadMercanciaViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaUnidadMercancia(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaUnidadMercancia,
                            filtros.paginacionDTO.IdGrilla, "CodigoUnidadMercancia", Request.QueryString["export"], Response, "Lista_de_UnidadMercancia_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaUnidadMercancia(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaUnidadMercancia, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoUnidadMercancia");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaUnidadMercancia, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarUnidadMercancia(RequestRegistrarUnidadMercanciaViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarUnidadMercancia(request);
