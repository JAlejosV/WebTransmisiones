...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers.Maestros
{
    ...
    {
        public ActionResult AdministrarMoneda()
        {
            return View("../Maestros/Moneda/AdministrarMoneda");
        }

        public ActionResult RegistroMoneda()
        {
            return View("../Maestros/Moneda/RegistroMoneda");
        }
        ...
        public ActionResult ConsultaMoneda(RequestBusquedaMonedaViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaMoneda(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaMoneda,
                            filtros.paginacionDTO.IdGrilla, "CodigoMoneda", Request.QueryString["export"], Response, "Lista_de_Moneda_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaMoneda(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaMoneda, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoMoneda");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaMoneda, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarMoneda(RequestRegistrarMonedaViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarMoneda(request);
