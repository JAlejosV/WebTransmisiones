...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers.Maestros
{
    ...
    {
        public ActionResult AdministrarTipoDocumento()
        {
            return View("../Maestros/TipoDocumento/AdministrarTipoDocumento");
        }

        public ActionResult RegistroTipoDocumento()
        {
            return View("../Maestros/TipoDocumento/RegistroTipoDocumento");
        }

        public ActionResult BuscarTipoDocumento()
        {
            return View("../Busqueda/BuscarTipoDocumento");
        }

        public ActionResult ConsultaTipoDocumento(RequestBusquedaTipoDocumentoViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaTipoDocumento(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaTipoDocumento,
                            filtros.paginacionDTO.IdGrilla, "CodigoTipoDocumento", Request.QueryString["export"], Response, "Lista_de_TipoDocumento_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaTipoDocumento(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaTipoDocumento, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoTipoDocumento");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaTipoDocumento, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarTipoDocumento(RequestRegistrarTipoDocumentoViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarTipoDocumento(request);
