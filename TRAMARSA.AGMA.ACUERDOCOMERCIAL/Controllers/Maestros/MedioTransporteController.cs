...
namespace TRAMARSA.AGMA.ACUERDOCOMERCIAL.Controllers.Maestros
{
    ...
    {
        public ActionResult AdministrarMedioTransporte()
        {
            return View("../Maestros/MedioTransporte/AdministrarMedioTransporte");
        }

        public ActionResult RegistroMedioTransporte()
        {
            return View("../Maestros/MedioTransporte/RegistroMedioTransporte");
        }

        public ActionResult BuscarMedioTransporte()
        {
            return View("../Busqueda/BuscarMedioTransporte");
        }

        public ActionResult ConsultaMedioTransporte(RequestBusquedaMedioTransporteViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaMedioTransporte(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaMedioTransporte,
                            filtros.paginacionDTO.IdGrilla, "CodigoMedioTransporte", Request.QueryString["export"], Response, "Lista_de_MedioTransporte_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaMedioTransporte(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaMedioTransporte, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoMedioTransporte");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaMedioTransporte, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarMedioTransporte(RequestRegistrarMedioTransporteViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarMedioTransporte(request);
