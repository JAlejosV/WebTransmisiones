...
{
    public class CondicionContratoController : Controller
    {
        public ActionResult AdministrarCondicionContrato()
        {
            return View("../Maestros/CondicionContrato/AdministrarCondicionContrato");
        }

        public ActionResult RegistroCondicionContrato()
        {
            return View("../Maestros/CondicionContrato/RegistroCondicionContrato");
        }

        public ActionResult BuscarCondicionContrato()
        {
            return View("../Busqueda/BuscarCondicionContrato");
        }

        public ActionResult ConsultaCondicionContrato(RequestBusquedaCondicionContratoViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaCondicionContrato(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaCondicionContrato,
                            filtros.paginacionDTO.IdGrilla, "CodigoCondicionContrato", Request.QueryString["export"], Response, "Lista_de_CondicionContrato_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaCondicionContrato(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaCondicionContrato, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoCondicionContrato");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaCondicionContrato, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarCondicionContrato(RequestRegistrarCondicionContratoViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarCondicionContrato(request);
