...
{
    public class CondicionPrecintoController : Controller
    {
        public ActionResult AdministrarCondicionPrecinto()
        {
            return View("../Maestros/CondicionPrecinto/AdministrarCondicionPrecinto");
        }

        public ActionResult RegistroCondicionPrecinto()
        {
            return View("../Maestros/CondicionPrecinto/RegistroCondicionPrecinto");
        }

        public ActionResult BuscarCondicionPrecinto()
        {
            return View("../Busqueda/BuscarCondicionPrecinto");
        }

        public ActionResult ConsultaCondicionPrecinto(RequestBusquedaCondicionPrecintoViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaCondicionPrecinto(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaCondicionPrecinto,
                            filtros.paginacionDTO.IdGrilla, "CodigoCondicionPrecinto", Request.QueryString["export"], Response, "Lista_de_CondicionPrecinto_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaCondicionPrecinto(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaCondicionPrecinto, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoCondicionPrecinto");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaCondicionPrecinto, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarCondicionPrecinto(RequestRegistrarCondicionPrecintoViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarCondicionPrecinto(request);
