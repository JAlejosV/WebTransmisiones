...
{
    public class RequerimientoServicioController : Controller
    {
        public ActionResult AdministrarRequerimientoServicio()
        {
            return View("../Maestros/RequerimientoServicio/AdministrarRequerimientoServicio");
        }

        public ActionResult RegistroRequerimientoServicio()
        {
            return View("../Maestros/RequerimientoServicio/RegistroRequerimientoServicio");
        }

        public ActionResult BuscarRequerimientoServicio()
        {
            return View("../Busqueda/BuscarRequerimientoServicio");
        }

        public ActionResult ConsultaRequerimientoServicio(RequestBusquedaRequerimientoServicioViewModel filtros, string requestExportar)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    if (!string.IsNullOrEmpty(Request.QueryString["export"]))
                    {
                        ...
                        var listaRespuesta = new MaestrosAgente().ConsultaRequerimientoServicio(filtros);
                        listaRespuesta.NroPagina = 1;
                        actionResult = HelperCtrl.ExportarExcel(listaRespuesta, listaRespuesta.ListaRequerimientoServicio,
                            filtros.paginacionDTO.IdGrilla, "CodigoRequerimientoServicio", Request.QueryString["export"], Response, "Lista_de_RequerimientoServicio_");

                    }
                    else
                    {
                        var listaCliente = new MaestrosAgente().ConsultaRequerimientoServicio(filtros);
                        if (listaCliente.Result.Satisfactorio)
                        {
                            ...
                            var res = Grid.toJSONFormat2(listaCliente.ListaRequerimientoServicio, filtros.paginacionDTO.GetNroPagina(),
                                listaCliente.TotalRegistros, totalPages, "CodigoRequerimientoServicio");
                            actionResult = Content(res);
                        }
                        else
                        {
                            actionResult = Content(Grid.toJSONFormat2(listaCliente.ListaRequerimientoServicio, 0, 0, 0, ""));
                        }
                    }
                }
                else
        ...
        public ActionResult RegistrarRequerimientoServicio(RequestRegistrarRequerimientoServicioViewModel request)
        {
            ActionResult actionResult = null;
            var manejadorLogEventos = new ManejadorLogEventos();
            try
            {
                ...
                var response = new MaestrosAgente().RegistrarRequerimientoServicio(request);
