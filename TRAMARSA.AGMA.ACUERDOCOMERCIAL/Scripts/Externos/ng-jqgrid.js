                        ...
                        scope.GetAtributosGrid = function () {
                            ...
                            var paramjqGrid = {
                                ...
                                gridComplete: function () {
                                    ...
                                    if (attrs.gridid == "grillaListaTransmisionNave") {
                                        var rows = jQuery("#" + attrs.gridid).jqGrid('getRowData')
                                        for (i = 0; i < rows.length; i++) {
                                            if (rows[i]["EstadoEnvioAduanas"] == "True" && rows[i]["EstadoTransmision"] == "False") {
                                                // $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_TotalContenedoresTransmision]").css({ color: "red" });
                                                var botonEditar = '<div style="width:100%;padding-left:0px; align:center"><a style="cursor:pointer"><img width="22"  height="22" src="/images/ErrorSunat.png" /></a></div>';
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").html("");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").attr("title", "");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").append(botonEditar);
                                            }

                                            if (rows[i]["EstadoEnvioAduanas"] == "False") {
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").html("");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_Editar]").html("");
                                            }

                                            if (rows[i]["EstadoEnvioAduanas"] == "True") {
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_Editar] button img").attr('src', '/images/buscar2.png').attr('width', '25').attr('height', '25');
                                            }

                                            if (rows[i]["EstadoEnvioAduanas"] == "True" && rows[i]["EstadoTransmision"] == "True") {
                                                var botonEditar = '<div style="width:100%;padding-left:0px"><a style="cursor:pointer"><img width="22"  height="22" src="/images/OKSunat.png" /></a></div>';
                                                //var imgHtmlLupa = '<div style="width:100%;padding-left:0px; align:center"><a style="cursor:pointer"><img width="16"  height="16" src="/images/buscar.png" /></a></div>';
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").html("");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").attr("title", "");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionNave_EstadoTransmision]").append(botonEditar);
                                            }
                                    ...
                                    if (attrs.gridid == "grillaListaTransmisionDocumento") {
                                        var rows = jQuery("#" + attrs.gridid).jqGrid('getRowData')
                                        for (i = 0; i < rows.length; i++) {
                                            if (rows[i]["EstadoEnvioAduanas"] == "True" && rows[i]["EstadoTransmision"] == "False") {
                                                // $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_TotalContenedoresTransmision]").css({ color: "red" });
                                                var botonEditar = '<div style="width:100%;padding-left:0px; align:center"><a style="cursor:pointer"><img width="22"  height="22" src="/images/ErrorSunat.png" /></a></div>';
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_EstadoTransmision]").html("");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_EstadoTransmision]").attr("title", "");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_EstadoTransmision]").append(botonEditar);
                                            }

                                            if (rows[i]["EstadoEnvioAduanas"] == "False") {
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_EstadoTransmision]").html("");
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_Editar]").html("");
                                            }

                                            if (rows[i]["EstadoEnvioAduanas"] == "True") {
                                                $($("#" + (i + 1), "#" + attrs.gridid)[0]).find("[aria-describedby=grillaListaTransmisionDocumento_Editar] button img").attr('src', '/images/buscar2.png').attr('width', '25').attr('height', '25');
                                            }
                                    ...
                                    if (attrs.idgrilla.toUpperCase() == "18B0EDD8-1287-4ED9-84BF-4FFC9FBFFCC7") {
                                        ...
                                        // if (!$rootScope.DatosFormulario.CheckCab ) { //JM
                                        ...
                                        /*} else{ //JM
                                            // validar inicio 
                                            if (Object.keys($rootScope.DatosFormulario.ListaCheck).length >0) {
                                                var ids =$('#' + attrs.gridid).jqGrid('getDataIDs');
                                                for (var p = 0; p < ids.length; p++) {
                                                   $('#' + attrs.gridid).jqGrid('setSelection', ids[p], false);
                                                }
                                                $("#cb_" + attrs.gridid).prop("checked", true);
                                            } 

                                        }*/
                                    }
                                    if (attrs.idgrilla.toUpperCase() == "1A0B5F06-BB91-4713-AA00-966EF96A2000") {

                                        var lista = $rootScope.DatosFormulario.AdministrarTransmisionNave.ListaCheck;
                                        if (lista != undefined) {
                                            var filtroLista = lista;
                                            if (Object.keys(filtroLista).length > 0) {
                                                var rowIds = $('#' + attrs.gridid).jqGrid('getDataIDs');
                                                for (var indice in filtroLista) {
                                                    for (var p = 0; p < rowIds.length; p++) {
                                                        var row = $('#' + attrs.gridid).jqGrid('getRowData', rowIds[p]);
                                                        if (row.CodigoItinerario == filtroLista[indice].CodigoItinerario) {
                                                            $('#' + attrs.gridid).jqGrid('setSelection', rowIds[p], false);
                                                            CountCheck++;
                                                            break;
                                                        }
                                    ...
                                    if (attrs.idgrilla.toUpperCase() == "1A0B5F06-BB91-4713-AA00-966EF96A2001") {

                                        var lista = $rootScope.DatosFormulario.AdministrarTransmisionDocumento.ListaCheck;
                                        if (lista != undefined) {
                                            var filtroLista = lista;
                                            if (Object.keys(filtroLista).length > 0) {
                                                var rowIds = $('#' + attrs.gridid).jqGrid('getDataIDs');
                                                for (var indice in filtroLista) {
                                                    for (var p = 0; p < rowIds.length; p++) {
                                                        var row = $('#' + attrs.gridid).jqGrid('getRowData', rowIds[p]);
                                                        if (row.CodigoDocumento == filtroLista[indice].CodigoDocumento) {
                                                            $('#' + attrs.gridid).jqGrid('setSelection', rowIds[p], false);
                                                            CountCheck++;
                                                            break;
                                                        }
