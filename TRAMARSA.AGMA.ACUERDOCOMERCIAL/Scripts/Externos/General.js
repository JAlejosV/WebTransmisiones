...
function ObtenerRoles() {
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.ListaRoles.length > 0) {
                resultado = data.ListaRoles;
            }
...
function ObtenerTerminalesByLinea(codLinea, codSucursal) {
    var resultado = [];
    $.ajax({
        url: "/Maestros/ListarTerminalPortuarioByLineaSucursal",
        type: "POST",
        headers: { __RequestVerificationToken: $('input[name=__RequestVerificationToken]').val() },
        data: "codigoLinea=" + codLinea + "&codigoSucursal=" + codSucursal,
        dataType: "json",
        cache: true,
        async: false,
        success: function (data) {
            if (data.TerminalPorturario.length > 0) {
                resultado = data.TerminalPorturario;
            }
...
function ConfiguracionSecciones(codigoPantalla, codigoLinea) {
    var responseData = new Object();
    $.ajax({
        url: "/Maestros/ObtenerConfiguracionLinea",
        type: "POST",
        headers: { __RequestVerificationToken: $('input[name=__RequestVerificationToken]').val() },
        data: "",
        dataType: "json",
        ...
        success: function (data) {
            ...
            if (data.ConfiguracionLineaPantallaSeccion.length > 0) {
                var lstConfiguracion = $.grep(data.ConfiguracionLinea, function (e) { return e.CodigoLinea == codigoLinea; });
                if (lstConfiguracion.length > 0) {
                    var lstConfiguracionPantalla = $.grep(data.ConfiguracionLineaPantallaSeccion, function (e) {
                        return e.CodigoPantalla == codigoPantalla &&
                          e.CodigoConfiguracion == lstConfiguracion[0].CodigoConfiguracion;
                    });
                    $.each(lstConfiguracionPantalla, function (x) {
                        if (this.TipoControl == "Label") {
                            $("div").find("[data-codigoseccion='" + this.CodigoSeccion + "']").text(this.SeccionTitulo);
                            if (this.SeccionVisible == false) {
                                $("div").find("[data-codigoseccion='" + this.CodigoSeccion + "']").parent().hide();
                            }
                            if (this.SeccionVisible == true) {
                                $("div").find("[data-codigoseccion='" + this.CodigoSeccion + "']").parent().show();
                            }
                        }
                        if (this.TipoControl == "Columna") {
                            var idGrilla = "";
                            if (codigoPantalla == "AdministracionConcesionLocal") {
                                idGrilla = "#grillaResultadoBusquedaACLocal";
                            }
                            if (codigoPantalla == "AdministracionConcesionEscalonada") {
                                idGrilla = "#grillaListaACEscalonado";
                            }


                            $(idGrilla).jqGrid('setLabel', this.CodigoSeccion, this.SeccionTitulo);
                            if (this.SeccionVisible == false) {
                                $(idGrilla).hideCol(this.CodigoSeccion);
                            }
                            if (this.SeccionVisible == true) {
                                $(idGrilla).showCol(this.CodigoSeccion);
                            }
                        }
                        if (this.TipoControl == "Seccion") {
                            var idGrilla = "";
                            if (codigoPantalla == "ConcesionLocal") {
                                idGrilla = "#grillaListaRate";
                            }
                            if (codigoPantalla == "ConcesionEscalonada") {
                                idGrilla = "#grillaAceListaRate";
                            }
                            if (this.CodigoSeccion == "RateAgreement") {
                                $(idGrilla).jqGrid('setLabel', "CodigoRA", this.SeccionTitulo);
                                if (this.SeccionVisible == false) {
                                    $('*[data-tipoconcesion=' + this.CodigoSeccion + ']').parent().parent().hide();
                                }
                                if (this.SeccionVisible == true) {
                                    $('*[data-tipoconcesion=' + this.CodigoSeccion + ']').parent().parent().show();
                                }
                            }

                            $("div").find("[data-codigoseccion='" + this.CodigoSeccion + "']").parent().parent().parent().find(".block_cab").find("span").text(this.SeccionTitulo);
                            if (this.SeccionVisible == false) {
                                $("div").find("[data-codigoseccion='" + this.CodigoSeccion + "']").parent().parent().parent().hide();
                            }
                            if (this.SeccionVisible == true) {
                                $("div").find("[data-codigoseccion='" + this.CodigoSeccion + "']").parent().parent().parent().show();
                            }
                        }
...
function HtmlCrearBoton_Aux(tipo, valor, EventoNgClick, atributosHtmlBoton) {
    ...
    if (tipo == "Estado") {
        salida = '<div style="width:100%;padding-left:0px"><a style="cursor:pointer"><img width="16" ' + atributosHtmlBoton + ' height="16" src="/img/' + imgEstado + '" /></a></div>';
    }
        ...
        salida = '<div style="width:100%;padding-left:30px"><input id="jqGridcheckBox" type="checkbox" class="jqGridcheckBoxcs"  ' + atrngclick + '" /><div>';
    }
    else {
        var atrngclick = '';
        if (!(EventoNgClick == undefined || EventoNgClick == "")) {
        ...
        salida = '<div style="width:100%;padding-left:10px"><a style="cursor:pointer;width:100%"><button title="' + strTooltip + '" ' + atributosHtmlBoton + ' ' + atrngclick + ' class="boton1Style botonpequenio"><img width="16" height="16" src="/images/' + strImg + '"></button></a></div>';
    }
...
function JqGridCrearBtn(cellvalue, options, tipo, rowId, rowId2, IdSufijo, rowId3, rowObject, nameId1) {
    ...
    if (tipo === "Modificar") {
        ...
        salida = '<div style="width:100%;padding-left:0px"><a style="cursor:pointer"><img width="16" height="16" src="/img/"' + imgEstado + ' alt="" dataval="' + rowId + '" ' + dataval2 + '/></a></div>';
    }
...
getPopupResponsive = function (m_settings) {
    ...
    $.ajax({
        ...
        success: function (datos) {
            objBox.html(datos);
            var htmlCabecera = '<div class="modalCabecera"><button type="button" class="close" data-dismiss="modal" aria-hidden="true" style="padding-right: 8px;padding-top: 2px;">x</button><h4 class="modal-title ui-dialog-titlebar ui-widget-header ui-corner-all ui-helper-clearfix" style="    font-family: Lucida Grande, Lucida Sans, Arial, sans-serif;    font-size: 13px;    cursor: move!important;    padding: .4em 1em;">' + settings["title"] + '</h4></div>';
...
function ReadOnlyForm(grillas, flagEditing, contentFechas) {
    $.each(grillas, function (a, b) {
        ...
        $.each(lstcontrolsButton, function (x, y) {
            ...
            if (lstcontrolsButton[x].title == "Anular") {
                if (flagEditing) {
